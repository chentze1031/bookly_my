...
class DbService {
  ...
  static Future<Database> _open() async {
    ...
    return openDatabase(path, version: 8, onCreate: (db, v) async {
      ...
      await db.execute('''CREATE TABLE quotation_seq(
        year INTEGER PRIMARY KEY,
        last_seq INTEGER NOT NULL DEFAULT 0
      )''');
      await db.execute('''CREATE TABLE delivery_order_seq(
        year INTEGER PRIMARY KEY,
        last_seq INTEGER NOT NULL DEFAULT 0
      )''');
      ...
      await db.execute('''CREATE TABLE purchase_order_seq(
        year INTEGER PRIMARY KEY,
        last_seq INTEGER NOT NULL DEFAULT 0
      )''');
      ...
      if (oldV < 4) {
        await db.execute('''CREATE TABLE IF NOT EXISTS quotation_seq(
          year INTEGER PRIMARY KEY,
          last_seq INTEGER NOT NULL DEFAULT 0
        )''');
      }
      ...
      if (oldV < 7) {
        await db.execute('''CREATE TABLE IF NOT EXISTS purchase_order_seq(
          year INTEGER PRIMARY KEY,
          last_seq INTEGER NOT NULL DEFAULT 0
        )''');
      }
  ...
  static Future<String> nextQuotNo() async {
    final d    = await db;
    final year = DateTime.now().year;
    await d.execute(
      'INSERT OR IGNORE INTO quotation_seq(year, last_seq) VALUES(?, 0)',
      [year],
    );
    await d.execute(
      'UPDATE quotation_seq SET last_seq = last_seq + 1 WHERE year = ?',
      [year],
    );
    final rows = await d.query('quotation_seq', where: 'year = ?', whereArgs: [year]);
    final seq  = (rows.first['last_seq'] as int?) ?? 1;
    return 'QT-$year-${seq.toString().padLeft(4, '0')}';
  }

  static Future<String> nextDoNo() async {
    final d    = await db;
    final year = DateTime.now().year;
    await d.execute(
      'INSERT OR IGNORE INTO delivery_order_seq(year, last_seq) VALUES(?, 0)',
      [year],
    );
    await d.execute(
      'UPDATE delivery_order_seq SET last_seq = last_seq + 1 WHERE year = ?',
      [year],
    );
  ...
  static Future<String> nextPoNo() async {
    final d    = await db;
    final year = DateTime.now().year;
    await d.execute(
      'INSERT OR IGNORE INTO purchase_order_seq(year, last_seq) VALUES(?, 0)',
      [year],
    );
    await d.execute(
      'UPDATE purchase_order_seq SET last_seq = last_seq + 1 WHERE year = ?',
      [year],
    );
    final rows = await d.query('purchase_order_seq', where: 'year = ?', whereArgs: [year]);
    final seq  = (rows.first['last_seq'] as int?) ?? 1;
    return 'PO-$year-${seq.toString().padLeft(4, '0')}';
  }
