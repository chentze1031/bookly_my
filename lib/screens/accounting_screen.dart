...
class _AccountingScreenState extends State<AccountingScreen>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // Always keep appState linked (safe to set again if already set at startup)
      acc.appState = context.read<AppState>();
      // Only init if not already loaded (avoids double-loading on tab switch)
      if (acc.arInvoices.isEmpty && acc.apBills.isEmpty && !acc.loading) {
        acc.init();
      }
    });
  }
...
class _ArDetailSheetState extends State<_ArDetailSheet> {
  ...
  bool _paying = false;
  String _payMethod = 'bank'; // 'cash' | 'bank'
              ...
              Row(children: [
                ...
                ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(
                    backgroundColor: _payMethod == 'cash' ? kGreen : kBlue,
                  ...
                  child: _paying
                    ...
                    : Text(t.record, style: const TextStyle(fontWeight: FontWeight.w700)),
                ),
              ]),
            ],
          ])),
...
class _ApDetailSheetState extends State<_ApDetailSheet> {
  ...
  Future<void> _recordPayment() async {
    ...
    if (amt <= 0) return;
    setState(() => _paying = true);
    // Always ensure appState linked so GL entries are written
    final acc = context.read<AccountingState>();
    acc.appState ??= context.read<AppState>();
            ...
            if (bill.status != InvoiceStatus.paid && bill.status != InvoiceStatus.void_) ...[
              ...
              Row(children: [
                ...
                ElevatedButton(
                  ...
                  child: _paying
                    ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                    : Text(t.pay, style: const TextStyle(fontWeight: FontWeight.w700)),
                ),
              ]),
            ],
          ])),
...
class _TrialBalanceTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (final e in bals.entries) {
      ...
      if (acc == null || e.value == 0) continue;
      final isNormalDr = acc.normal == 'Dr';
      if (e.value > 0) {
        // Positive balance = normal side
        if (isNormalDr) totalDr += e.value;
        else            totalCr += e.value;
      } else {
        // Negative balance = opposite side
        if (isNormalDr) totalCr += e.value.abs();
        else            totalDr += e.value.abs();
      }
    }
            ...
            return GestureDetector(
              ...
              child: Container(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Text(fmt.format(e.value.abs()), maxLines: 1, overflow: TextOverflow.ellipsis,
...
class _PayMethodBtn extends StatelessWidget {
  ...
  final Color color;
  final VoidCallback onTap;
  const _PayMethodBtn({required this.icon, required this.label,
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: selected ? color : kBorder, width: 1.5),
