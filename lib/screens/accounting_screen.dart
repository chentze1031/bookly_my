...
class _AccountingScreenState extends State<AccountingScreen>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) =>
      context.read<AccountingState>().init());
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      Container(
        ...
        child: Row(children: [
          _SummaryChip(label: 'Total AR',  value: 'RM ${fmt.format(acc.totalReceivable)}', color: kGreen),
          _SummaryChip(label: 'Overdue',   value: 'RM ${fmt.format(acc.totalOverdueAr)}',  color: kRed),
          _SummaryChip(label: 'Invoices',  value: '${acc.arInvoices.length}',              color: kText),
        ]),
      ),
      ...
      Expanded(child: list.isEmpty
        ? _EmptyState(label: 'No invoices', onAdd: () => _showArForm(context))
        : ListView.separated(
      ...
      builder: (_, scroll) => Container(
        ...
        child: Column(children: [
          ...
          Expanded(child: ListView(controller: scroll, padding: const EdgeInsets.all(20), children: [
            ...
            // Amounts
            Row(children: [
              Expanded(child: _AmountBox(label: 'Subtotal',   value: fmt.format(inv.subtotal),   color: kText)),
              const SizedBox(width: 10),
              Expanded(child: _AmountBox(label: 'SST',        value: fmt.format(inv.sstAmount),  color: kMuted)),
            ]),
            ...
            const SizedBox(height: 10),
            _AmountBox(label: 'Balance Due', value: 'RM ${fmt.format(inv.balance)}',
              color: inv.balance > 0 ? kRed : kGreen, large: true),

            const SizedBox(height: 16),
            _DetailRow('Issue Date', inv.issueDate),
            _DetailRow('Due Date',   inv.dueDate),
            if (inv.notes?.isNotEmpty == true) _DetailRow('Notes', inv.notes!),

            // Items
            if (inv.items.isNotEmpty) ...[
              const SizedBox(height: 16),
              const Text('ITEMS', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: kMuted, letterSpacing: 0.6)),
              const SizedBox(height: 8),
            ...
            if (inv.status != InvoiceStatus.paid && inv.status != InvoiceStatus.void_) ...[
              ...
              const Text('RECORD PAYMENT', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: kMuted, letterSpacing: 0.6)),
              const SizedBox(height: 8),
              Row(children: [
                ...
                ElevatedButton(
                  ...
                  child: _paying
                    ...
                    : const Text('Record', style: TextStyle(fontWeight: FontWeight.w700)),
                ),
              ]),
            ],
          ])),
...
class _ArInvoiceFormState extends State<_ArInvoiceForm> {
  ...
  Future<void> _save() async {
    ...
    final inv = ArInvoice(
      ...
      items:        _items.where((i) => i.desc.isNotEmpty).map((i) => ArInvoiceItem(
        description: i.desc,
        qty:         double.tryParse(i.qty) ?? 1,
        unitPrice:   double.tryParse(i.price) ?? 0,
        amount:      i.amount,
      )).toList(),
    );
          ...
          Padding(
            ...
            child: Row(children: [
              Container(width: 40, height: 4,
                decoration: BoxDecoration(color: kBorder, borderRadius: BorderRadius.circular(99))),
              const Spacer(),
              Text(widget.invoice == null ? 'New Invoice' : 'Edit Invoice',
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: _saving
                    ...
                    : Text(widget.invoice == null ? 'Create Invoice' : 'Save Changes',
...
// ── Line item helpers ─────────────────────────────────────────────────────────
class _LineItem {
  String desc = '', qty = '1', price = '0';
  _LineItem({this.desc = '', this.qty = '1', this.price = '0'});
  double get amount => (double.tryParse(qty) ?? 0) * (double.tryParse(price) ?? 0);
}
...
class _LineItemRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 6),
        Row(children: [
          Expanded(child: _miniField('Qty', item.qty, (v) { item.qty = v; onChanged(); })),
          const SizedBox(width: 8),
          Expanded(flex: 2, child: _miniField('Unit Price', item.price, (v) { item.price = v; onChanged(); })),
          const SizedBox(width: 8),
          Text('RM ${fmt.format(item.amount)}',
            style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: kText)),
        ]),
  ...
  Widget _miniField(String hint, String val, ValueChanged<String> onChg) => TextField(
    onChanged: onChg,
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      Container(
        ...
        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
        child: Row(children: [
          _SummaryChip(label: 'Total AP',  value: 'RM ${fmt.format(acc.totalPayable)}',   color: kRed),
          _SummaryChip(label: 'Overdue',   value: 'RM ${fmt.format(acc.totalOverdueAp)}', color: Colors.orange),
          _SummaryChip(label: 'Bills',     value: '${acc.apBills.length}',                color: kText),
        ]),
      ),
      if (aging.total > 0) _AgingBar(aging: aging, isAp: true),
      _FilterRow(selected: _filter, onSelect: (s) => setState(() => _filter = s)),
      Expanded(child: list.isEmpty
        ? _EmptyState(label: 'No bills', onAdd: () => _showApForm(context))
        : ListView.separated(
            padding: const EdgeInsets.fromLTRB(16, 8, 16, 100),
            itemCount: list.length,
            separatorBuilder: (_, __) => const SizedBox(height: 8),
      ...
      builder: (_, scroll) => Container(
        ...
        child: Column(children: [
          ...
          Expanded(child: ListView(controller: scroll, padding: const EdgeInsets.all(20), children: [
            ...
            const SizedBox(height: 10),
            _AmountBox(label: 'Balance Due', value: 'RM ${fmt.format(bill.balance)}',
              color: bill.balance > 0 ? Colors.orange : kGreen, large: true),
            const SizedBox(height: 16),
            _DetailRow('Issue Date', bill.issueDate),
            _DetailRow('Due Date',   bill.dueDate),
            if (bill.category != null) _DetailRow('Category', bill.category!),
            if (bill.notes?.isNotEmpty == true) _DetailRow('Notes', bill.notes!),
            ...
            if (bill.status != InvoiceStatus.paid && bill.status != InvoiceStatus.void_) ...[
              ...
              Row(children: [
                Expanded(child: TextField(
                  ...
                  keyboardType: const TextInputType.numberWithOptions(decimal: true),
                  decoration: InputDecoration(
                    hintText: 'Amount paid', prefixText: 'RM ',
                    filled: true, fillColor: kBg,
                    border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: kBorder)),
                    enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: kBorder)),
                    contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
    ...
    builder: (_, scroll) => Container(
      ...
      child: Column(children: [
        Padding(
          ...
          child: Row(children: [
            const Spacer(),
            Text(widget.bill == null ? 'New Bill' : 'Edit Bill',
              style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 17, color: kText)),
            const Spacer(),
            TextButton(onPressed: () => Navigator.pop(context),
              child: const Text('Cancel', style: TextStyle(color: kMuted))),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 10),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(balanced ? 'Books are balanced' : 'Books are NOT balanced',
...
class _GeneralLedgerTabState extends State<_GeneralLedgerTab> {
  ...
  Widget build(BuildContext context) {
    ...
    final activeAccounts = bals.entries
      ...
      .toList()
      ..sort((a, b) => accounts[a.key]!.type.compareTo(accounts[b.key]!.type));

    // Filter txs for selected account
    final selectedTxs = _selectedAcc == null ? <_GlEntry>[] :
      app.txs.expand((tx) => tx.entries
    ...
    return Row(children: [
      ...
      Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Padding(
            ...
            child: Text('ACCOUNTS', style: TextStyle(fontSize: 9, fontWeight: FontWeight.w800, color: kMuted, letterSpacing: 0.6)),
          ),
      ...
      Expanded(child: _selectedAcc == null
        ? const Center(child: Text('← Select an account', style: TextStyle(color: kMuted, fontSize: 14)))
        : Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
              color: kSurface,
            ...
            Expanded(child: selectedTxs.isEmpty
              ? const Center(child: Text('No entries', style: TextStyle(color: kMuted)))
              : ListView.separated(
...
class _AgingBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('AGING ANALYSIS', style: TextStyle(fontSize: 9, fontWeight: FontWeight.w800, color: kMuted, letterSpacing: 0.6)),
        const SizedBox(height: 8),
        Row(children: [
