    ...
    return SingleChildScrollView(
      child: Column(
        children: [
          ...
          // ── Quick actions ───────────────────────────────────────────────────
          // FIX: _QuickBtn must NOT return Expanded internally when used inside Row+Expanded.
          // Each button is wrapped in Expanded here, and _QuickBtn is a plain Container.
          Padding(
...
// FIX: _QuickBtn is now a plain widget (no longer wraps itself in Expanded).
// The caller (HomeScreen) wraps each in Expanded inside the Row.
