      ...
      builder: (_, scroll) => Container(
        ...
        child: Column(children: [
          ...
          Expanded(child: ListView(controller: scroll, padding: const EdgeInsets.all(20), children: [
            // Stock adjust
            _StockAdjustCard(item: item, inv: inv),
...
// ── Stock adjust card (Stock In / Stock Out / Set) ───────────────────────────
class _StockAdjustCard extends StatefulWidget {
  final InventoryItem item;
  final InventoryState inv;
  const _StockAdjustCard({required this.item, required this.inv});
...
class _StockAdjustCardState extends State<_StockAdjustCard> {
  ...
  Future<void> _apply() async {
    ...
    if (val == null || val < 0) {
      setState(() => _err = 'Enter a valid number');
      return;
    }
    setState(() { _busy = true; _err = null; });
    try {
      if (_mode == 'in')  await widget.inv.stockIn(widget.item.id, val);
      if (_mode == 'out') await widget.inv.stockOut(widget.item.id, val);
      if (_mode == 'set') await widget.inv.setQty(widget.item.id, val);
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        const SizedBox(height: 4),
        Text('${fmt.format(widget.item.qty)} ${widget.item.unit}',
          style: TextStyle(fontSize: 28, fontWeight: FontWeight.w900,
            color: widget.item.isOutOfStock ? kRed : widget.item.isLowStock ? Colors.orange : kText)),
        const SizedBox(height: 14),

        // Mode selector
        Row(children: [
          for (final m in [('in', '📥 Stock In', kGreen), ('out', '📤 Stock Out', kRed), ('set', '✏️ Set', kMuted)])
        ...
        Row(children: [
          Expanded(child: TextField(
            ...
            decoration: InputDecoration(
              hintText: _mode == 'set' ? 'New quantity' : 'Quantity',
...
class _ItemFormSheetState extends State<_ItemFormSheet> {
  ...
  Future<void> _save() async {
    if (_name.text.trim().isEmpty) {
      setState(() => _saveError = 'Product name is required.');
      return;
    }
          ...
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                _Field(label: 'SKU / BARCODE', child: Row(children: [
...
class _BarcodeScannerScreenState extends State<_BarcodeScannerScreen> {
  ...
  void _onDetect(BarcodeCapture capture) {
    ...
    Navigator.pop(context, _lastCode);
  }
...
// Corner accent widget for scan overlay
