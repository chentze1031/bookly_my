  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      constraints: BoxConstraints(
          maxHeight: MediaQuery.of(context).size.height * 0.88 - MediaQuery.of(context).viewInsets.bottom),
      child: Column(children: [
...
class _FullInvoiceSheetState extends State<FullInvoiceSheet> {
  ...
  Future<void> _saveInvoice() async {
    ...
    try {
      ...
      // ── Sync GL + AR ──────────────────────────────────────────────────────
      try {
        final total = _items.fold<double>(0, (s, item) {
          final qty   = double.tryParse(item['qty']   ?? '1') ?? 1;
          final price = double.tryParse(item['price'] ?? '0') ?? 0;
          return s + qty * price;
        });

        // Determine catId based on payment status
        final catId = switch (_invPayStatus) {
          'cash'  => 'inv_cash',
          'bank'  => 'inv_bank',
          _       => 'inv_unpaid',
        };
        ...
        // Write Transaction → Reports + GL
        if (cat != null && total > 0) {
          await context.read<AppState>().addOrUpdateTx(Transaction(
            ...
            sstKey:       'none',
            sstMYR:       0,
            descEN:       '${_customer?.name ?? ''} – Invoice $_invNo',
            descZH:       '${_customer?.name ?? ''} – 发票 $_invNo',
            date:         _invDate,
            entries:      cat.mkEntries(total),
          ));
        }

        // Write ArInvoice → Accounting → Receivable
        final acc = context.read<AccountingState>();
        final arInv = ArInvoice(
          id:           _invNo.hashCode.abs(),
          invNo:        _invNo,
          ...
          sstAmount:    0,
          total:        total,
          amountPaid:   _invPayStatus != 'unpaid' ? total : 0,
          status:       _invPayStatus != 'unpaid' ? InvoiceStatus.paid : InvoiceStatus.sent,
          notes:        _notes,
  ...
  // After sharing, automatically saves the invoice and closes the sheet.
  Future<void> _share() async {
    ...
    try {
      ...
      // ── Auto-save then close after sharing ───────────────────────────────
      if (mounted) {
        await _saveInvoice();
        // _saveInvoice already pops the sheet on success, so we only close
        // here if it didn't (e.g. already saved and mounted is still true).
        if (mounted) {
          final nav = Navigator.of(context);
          if (nav.canPop()) nav.pop();
        }
        ...
        Expanded(
          child: ListView(
            ...
            children: [
              ...
              _PayStatusPicker(
                value: _invPayStatus,
                onChanged: (v) => setState(() => _invPayStatus = v),
                lang: context.read<AppState>().settings.lang,
              ),
              const SizedBox(height: 14),
...
// ── Payment Status Picker (used in Invoice form) ──────────────────────────────
class _PayStatusPicker extends StatelessWidget {
  final String value;
  final ValueChanged<String> onChanged;
  final String lang;
  const _PayStatusPicker({required this.value, required this.onChanged, required this.lang});
        ...
        for (int i = 0; i < _opts.length; i++) ...[
          ...
          Expanded(child: GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color:  value == _opts[i].$1 ? _color(_opts[i].$1) : kSurface,
                border: Border.all(
                  color: value == _opts[i].$1 ? _color(_opts[i].$1) : kBorder,
                  width: 1.5,
                ),
                borderRadius: BorderRadius.circular(10),
              ),
              child: Column(children: [
                Text(_opts[i].$2, style: const TextStyle(fontSize: 18)),
                const SizedBox(height: 3),
                Text(lang == 'zh' ? _opts[i].$4 : _opts[i].$3,
                  textAlign: TextAlign.center,
                  style: TextStyle(
                    fontSize: 10, fontWeight: FontWeight.w700,
                    color: value == _opts[i].$1 ? Colors.white : kMuted,
                  )),
              ]),
            ),
          )),
