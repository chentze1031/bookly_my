        ...
        Expanded(
          child: ListView(
            ...
            children: [
              DashedBtn(
                  label: '+ ${t.newCust}',
                  // FIX: removed 'const' — Customer constructor is not const
                  onTap: () => setState(
...
class _FullInvoiceSheetState extends State<FullInvoiceSheet> {
  // Invoice No starts empty; initState loads the next sequential number from DB
  String _invNo   = '';
  ...
  // FIX: removed 'const' keyword — Customer() is not a const constructor
  ...
  // ── Load next sequential invoice number from DB on open ───────────────────
  @override
  void initState() {
    super.initState();
  ...
  // FIX: This function was completely missing, causing the UI to freeze
  // because _sharing was set to true but never reset to false.
  Future<void> _share() async {
    ...
    try {
      ...
      // Sanitise invoice number for use as filename
      ...
      // CRITICAL: always reset _sharing so the UI is never permanently locked
