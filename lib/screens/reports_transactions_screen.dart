...
class _ReportsState extends State<ReportsScreen> {
  ...
  // Financial statements (P&L + Balance Sheet) → PDF.
  Future<void> _exportFinancials() async {
    final app = context.read<AppState>();
    final zh  = app.settings.lang == 'zh';
    final filtered = _month.isNotEmpty ? app.txs.where((tx) => tx.date.startsWith(_month)).toList() : app.txs;
    final bal = app.computeBalances(filtered);
    double b(String k) => bal[k] ?? 0;
    final rev = b('4010') + b('4020') + b('4030');
    final cogs = b('5010'); final grossP = rev - cogs;
    final opex = bal.entries.where((e) => e.key.startsWith('5') && e.key != '5010')
        .fold<double>(0, (s, e) => s + e.value);
    final net = grossP - opex;
    final all = app.computeBalances();
    final cash = all['1020'] ?? 0, ar = all['1100'] ?? 0, invv = all['1200'] ?? 0;
    final totalA = cash + ar + invv; final ap = all['2010'] ?? 0;
    final payrollLiab = all.entries.where((e) => e.key.startsWith('2') && e.key != '2010')
        .fold<double>(0, (s, e) => s + e.value);
    final equity = totalA - ap - payrollLiab;
    final period = _month.isNotEmpty ? _month : (zh ? '全部时间' : 'All time');

    final bytes = await generateTableReportPdf(
      co: app.settings,
      title: zh ? '财务报表' : 'Financial Statements',
      subtitle: '${zh ? '期间' : 'Period'}: $period',
      headers: [zh ? '项目' : 'Item', zh ? '金额 (RM)' : 'Amount (RM)'],
      flex: [6, 3], rightCols: [false, true],
      rows: [
        [zh ? '— 损益表 —' : '— Profit & Loss —', ''],
        [zh ? '总收入' : 'Revenue', _amt(rev)],
        [zh ? '商品成本' : 'Cost of Goods', _amt(cogs)],
        [zh ? '毛利润' : 'Gross Profit', _amt(grossP)],
        [zh ? '运营费用' : 'Operating Expenses', _amt(opex)],
        [zh ? '净利润' : 'Net Income', _amt(net)],
        ['', ''],
        [zh ? '— 资产负债表（当前）—' : '— Balance Sheet (current) —', ''],
        [zh ? '银行存款' : 'Cash in Bank', _amt(cash)],
        [zh ? '应收账款' : 'Accounts Receivable', _amt(ar)],
        [zh ? '库存' : 'Inventory', _amt(invv)],
        [zh ? '资产总额' : 'Total Assets', _amt(totalA)],
        [zh ? '应付账款' : 'Accounts Payable', _amt(ap)],
        if (payrollLiab.abs() > 0.005) [zh ? '薪资与法定缴款' : 'Payroll & Statutory Payable', _amt(payrollLiab)],
      ],
      totals: [zh ? '净资产' : 'Net Worth (Equity)', _amt(equity)],
      note: zh ? '由 Bookly MY 生成，仅供参考。' : 'Generated by Bookly MY for reference.',
    );
    await _shareFile(bytes, 'Financials_${period.replaceAll(RegExp(r"[^A-Za-z0-9]"), "_")}.pdf', 'application/pdf');
  }

  // Transactions → CSV (opens in Excel).
  Future<void> _exportCsv() async {
    ...
    for (final tx in sorted) {
      ...
      buf.writeln('${tx.date},${tx.type},${esc(cat)},${esc(desc)},${tx.amountMYR.toStringAsFixed(2)},${tx.sstMYR.toStringAsFixed(2)}');
    }
    final period = _month.isNotEmpty ? _month : 'all';
    await _shareFile(buf.toString().codeUnits, 'Transactions_$period.csv', 'text/csv');
  }
  ...
  Widget build(BuildContext context) {
    ...
    final filtered = (sub.hasAccess && _month.isNotEmpty)
    ...
    // Operating expenses = every expense account (5xxx) except COGS (5010).
    // Prefix-based so payroll employer-statutory accounts (5101-5103) are counted.
    final totalOpEx = bal.entries.where((e) => e.key.startsWith('5') && e.key != '5010')
        .fold<double>(0, (s, e) => s + e.value);
    ...
    // Payroll & statutory payables (2110-2160) accrued from payslips.
    final payrollLiab = allBal.entries.where((e) => e.key.startsWith('2') && e.key != '2010')
        .fold<double>(0, (s, e) => s + e.value);
    final equity   = totalA - ap - payrollLiab;
      ...
      child: Column(
        children: [
          ...
          Container(
            ...
            child: Row(children: [
              ...
              _ViewTab(
                ...
                onTap: () => sub.hasAccess ? setState(() => _view = 'sst') : showSubSheet(context)),
            ]),
          ),
          const SizedBox(height: 10),

          // Export (Phase 4 #22, Pro)
          Align(
            alignment: Alignment.centerRight,
            child: GestureDetector(
              onTap: () => sub.hasAccess ? _showExportMenu() : showSubSheet(context),
              child: Container(
          ...
          if (_view == 'bs') _BSView(t: t,
            ...
            ap: ap, payrollLiab: payrollLiab, equity: equity),
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: Row(children: [
                Container(width: 40, height: 40, decoration: BoxDecoration(color: kGreenBg, borderRadius: BorderRadius.circular(10)),
...
class _BSView extends StatelessWidget {
  final L10n t;
  final double cash, ar, inv, totalA, ap, payrollLiab, equity;
  const _BSView({required this.t, required this.cash, required this.ar,
    required this.inv, required this.totalA, required this.ap,
    required this.payrollLiab, required this.equity});
  @override
  Widget build(BuildContext context) => Column(children: [
    _BSBlock(label: t.assets, total: totalA, color: kGreen, bgColor: kGreenBg,
      items: [(t.cashBank, cash), (t.ar, ar), (t.inventory, inv)]),
    const SizedBox(height: 10),
    _BSBlock(label: t.liab, total: ap + payrollLiab, color: kRed, bgColor: kRedBg,
      items: [
        (t.ap, ap),
        if (payrollLiab.abs() > 0.005)
          (tr(t.lang, 'Payroll & Statutory', '薪资与法定缴款', 'Gaji & Statutori'), payrollLiab),
      ]),
    const SizedBox(height: 10),
    Container(
