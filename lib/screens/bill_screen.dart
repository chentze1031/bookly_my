...
const _billTypes = [
  ...
  _BillType('other_expense','💸', 'Other Expense',         '其他支出'),
];
...
class _BillFormSheetState extends State<BillFormSheet> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      // saveApBill() writes BOTH the AP record AND the GL Transaction
      // (Phase 1: Dr Expense / Cr AP).
      // Do NOT also call app.addOrUpdateTx() — that caused duplicate entries.
      final acc = context.read<AccountingState>();
      acc.appState ??= context.read<AppState>();

      final billId = DateTime.now().millisecondsSinceEpoch;
      ...
      await acc.saveApBill(ApBill(
        ...
        category:     _type.id,  // use id so _billCatId() maps correctly
      ));

      // ── Phase 2 GL if already paid ────────────────────────────────────────
      // (Phase 2: Dr AP / Cr Bank) — balance-sheet-only, no P&L impact.
      if (_payStatus != _PayStatus.unpaid) {
        await acc.recordApPayment(billId, amt);
      }
  ...
  Widget _buildGlPreview(NumberFormat fmt, String lang) {
    final amt = double.tryParse(_amtCtrl.text) ?? 0;
    if (amt <= 0) return const SizedBox.shrink();
    final expLabel = _type.label(lang);
    return Container(
  ...
  Widget _glRow(String dc, String name, String val) => Padding(
