...
// ─── Shared payslip PDF builder ────────────────────────────────────────────────
// Single source of truth for the payslip layout. Both the Salary sheet and the
// Payroll-History detail view call this so the exported PDF is byte-for-byte the
// same design. Net pay = gross - totDed - pcb (PCB affects net but is not itemised).
Future<Uint8List> buildPayslipPdfBytes({
          ...
          child: pw.Padding(
            ...
            child: pw.Column(
              ...
              children: [
                ...
                // ── Summary stats row ───────────────────────────────────────
                pw.Row(children: [
                  statBox('GROSS EARNINGS', fmtMYR(gross)),
                  pw.SizedBox(width: 8),
                  statBox('TOTAL DEDUCTIONS', '(${fmtMYR(totDed)})'),
                  pw.SizedBox(width: 8),
                  statBox('NET PAY', fmtMYR(netPay), highlight: true),
                ]),
                    ...
                    pw.Expanded(
                      child: pw.Column(
                        ...
                        children: [
                          sHead('DEDUCTIONS'),
                          if (useEPF)
                            earningRow('EPF (Employee 11%)', fmtMYR(eeEPF)),
                          if (useSOCSO)
                            earningRow('SOCSO (Employee)', fmtMYR(eeSSO)),
                          if (useEIS)
                            earningRow('EIS (Employee)', fmtMYR(eeEIS)),
                          ...dedRows.map((d) {
                ...
                if (useEPF || useSOCSO || useEIS) ...[
                  ...
                  pw.Container(
                    ...
                    child: pw.Column(children: [
                      ...
                      // Rows
                      if (useEPF)
                        _statRow('KWSP / EPF', fmtMYR(eeEPF),
                            fmtMYR(erEPF), cDark, cRule, body, mono),
                      if (useSOCSO)
                        _statRow('PERKESO / SOCSO', fmtMYR(eeSSO),
                            fmtMYR(erSSO), cDark, cRule, body, mono),
                      if (useEIS)
                        _statRow('SIP / EIS', fmtMYR(eeEIS),
                            fmtMYR(erEIS), cDark, cRule, body, mono),
                      ...
                      pw.Container(
                        ...
                        child: pw.Row(
                          ...
                          children: [
                            pw.Text(
                                'Total employer cost: ${fmtMYR(erCost)}',
                                style: pw.TextStyle(
                                    fontSize: 7.5,
                                    color: cMuted,
                                    fontWeight: pw.FontWeight.bold)),
...
class FullPayrollSheet extends StatefulWidget {
  ...
  const FullPayrollSheet({super.key, this.prefill});
...
class _PayrollSheetState extends State<FullPayrollSheet> {
  ...
  double get _netPay => _gross - _totDedFull;
  double get _erCost => _gross + _erEPF + _erSSO + _erEIS;
  double get _pcb    => _usePCB ? calcPcbMonthly(_gross) : 0;
  double get _totDedFull => _totDed + _pcb;
  ...
  // ── Generate PDF payslip bytes (delegates to shared builder) ───────────────
  Future<Uint8List> _buildPayslipPdf(AppState app) => buildPayslipPdfBytes(
        app: app,
        emp: _emp!,
        month: _month,
        year: _year,
...
/// Calculates monthly PCB/MTD deduction based on annual chargeable income.
/// Uses Malaysia 2024 tax rate schedule.
double calcPcbMonthly(double monthlyGross, {bool isMale = true}) {
  // Annualise
  ...
  const brackets = [
    ...
    (200000.0, 0.25),
    (400000.0, 0.26),
    (double.infinity, 0.28),
  ];

  double remaining = chargeable.toDouble();
  for (final b in brackets) {
    final slice = b.$1.isInfinite ? remaining : b.$1.clamp(0, remaining);
    tax += slice * b.$2;
    remaining -= slice;
...
// ── Monthly net pay chart ─────────────────────────────────────────────────────
class _MonthlyChart extends StatelessWidget {
  final List<Map<String, dynamic>> records;
  final List<String> months;
  const _MonthlyChart({required this.records, required this.months});

  double _netOf(Map<String, dynamic> r) {
    final gross = (r['earnings'] as List)
        .fold<double>(0, (s,e) => s + (double.tryParse((e as Map)['amount']??'0')??0));
    final ded = (r['deductions'] as List)
        .fold<double>(0, (s,e) => s + (double.tryParse((e as Map)['amount']??'0')??0));
    final epf   = r['useEPF']   == true ? epfEe(gross)   : 0.0;
    final socso = r['useSOCSO'] == true ? socsoEe(gross) : 0.0;
    final eis   = r['useEIS']   == true ? eisEe(gross)   : 0.0;
    return gross - ded - epf - socso - eis;
  }
  ...
  Widget build(BuildContext context) {
    ...
    for (final r in records) {
      final mn = r['month'] as int;
      byMonth[mn] = (_netOf(r));
    }
    final maxVal = byMonth.values.isEmpty ? 1.0 : byMonth.values.reduce((a,b)=>a>b?a:b);
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(14)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('MONTHLY NET PAY', style: TextStyle(
        ...
        SizedBox(
          ...
          child: Row(
            ...
            children: List.generate(12, (i) {
              final mn  = i + 1;
              final val = byMonth[mn] ?? 0;
              final pct = maxVal > 0 ? val / maxVal : 0.0;
              return Expanded(child: Padding(
                padding: const EdgeInsets.symmetric(horizontal: 2),
                child: Column(mainAxisAlignment: MainAxisAlignment.end, children: [
                  if (val > 0) Text(
                    'RM${(val/1000).toStringAsFixed(1)}k',
                    style: const TextStyle(fontSize: 7, color: kMuted)),
                  const SizedBox(height: 2),
                  AnimatedContainer(
                    duration: const Duration(milliseconds: 400),
                    height: (pct * 52).clamp(2, 52),
                    decoration: BoxDecoration(
                      color: val > 0 ? kGreen : kBorder,
                      borderRadius: BorderRadius.circular(4)),
                  ),
                  const SizedBox(height: 4),
                  Text(months[i], style: const TextStyle(fontSize: 7, color: kMuted)),
                ]),
              ));
            }),
          ),
...
// ════════════════════════════════════════════════════════════════════════════
// SHARED PREFS HELPER (used by history screen)
// ════════════════════════════════════════════════════════════════════════════
