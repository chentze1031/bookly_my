...
void main() async {
  ...
  final accountingState = AccountingState();

  await Future.wait([appState.init(), subState.init()]);

  // Inject AppState immediately so GL sync works from ALL entry points
  // (AddExpense → bill, HomeScreen → Invoice, AccountingScreen, etc.)
  // Previously this was only done lazily inside AccountingScreen.initState(),
  // which meant payments from outside that tab silently skipped GL writes.
  accountingState.appState = appState;
